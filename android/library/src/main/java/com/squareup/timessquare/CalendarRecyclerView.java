...
public class CalendarRecyclerView extends RecyclerView {
    ...
    private final CalendarRecyclerView.MonthAdapter adapter;
    ...
    private boolean showInactiveDays;
    private boolean showOtherDaysOfMonth;
    private int headerTextColor;
    private Typeface titleTypeface;
    private Typeface dateTypeface;

    private OnDateSelectedListener dateListener;
    private OnDateRangeSelectListener dateRangeListener;
    ...
    private OnBindMonthViewListener onBindMonthViewListener;
    private MonthView.CellViewAdapter monthCellViewAdapter;

    /** aspect ratio of day cell view */
    private float aspectRatio = 1.0f;

    /** clock proxy, providing current timestamp */
    private ITimeClock clock = new DefaultClock();
    ...
    public CalendarRecyclerView(Context context, AttributeSet attrs) {
        ...
        dividerColor =
                a.getColor(R.styleable.CalendarPickerView_tsquare_dividerColor, res.getColor(R.color.transparent));
        dayBackgroundResId =
                a.getResourceId(R.styleable.CalendarPickerView_tsquare_dayBackground, R.drawable.calendar_bg_selector);
        dayTextColorResId =
                a.getResourceId(R.styleable.CalendarPickerView_tsquare_dayTextColor, R.color.calendar_text_selector);
        ...
        headerTextColor = a.getColor(R.styleable.CalendarPickerView_tsquare_headerTextColor,
                res.getColor(R.color.calendar_text_active));
        showInactiveDays = a.getBoolean(R.styleable.CalendarPickerView_tsquare_showInactiveDays, false);
        a.recycle();
        ...
        this.addItemDecoration(dividerLine);
        ...
        // todo touch listener confilicts with calenday day click
//        this.addOnItemTouchListener(touchListener);
        // *********** sticky header ends

        monthCellViewAdapter = new DefaultCellViewAdapter();

        timeZone = TimeZone.getDefault();
        locale = Locale.getDefault();
        today = Calendar.getInstance(timeZone, locale);
        today.setTime(new Date(clock.currentTimeMillis()));
    ...
    public FluentInitializer init(Date minDate, Date maxDate, TimeZone timeZone, Locale locale) {
        if (minDate == null || maxDate == null) {
            //IllegalArgumentException("minDate and maxDate must be non-null.  " + dbg(minDate, maxDate));
        }
    ...
    private void scrollToSelectedMonth(final int selectedIndex, final boolean smoothScroll) {
        post(new Runnable() {
            ...
            public void run() {
                ...
                if (smoothScroll) {
                    layoutManager.smoothScrollToPosition(CalendarRecyclerView.this, new State(), selectedIndex);
                } else {
    ...
    private class CellClickedListener implements MonthView.Listener {
        ...
        public void handleClick(MonthCellDescriptor cell) {
            ...
            if (cellClickInterceptor != null && cellClickInterceptor.onCellClicked(cell, clickedDate)) {
                return;
            }
    ...
    private boolean doSelectDate(Date date, MonthCellDescriptor cell) {
        ...
        switch (selectionMode) {
            case RANGE:
                if (selectedCals.size() == 1) { // weipf: unselect the current one if it is already in selected
                    // todo maybe move to below
                    Calendar currSelectCal = selectedCals.get(0);
                    if (isSameDay(currSelectCal, newlySelectedCal)) {
                        clearOldSelections();
                        if (dateRangeListener != null) {
                            dateRangeListener.onRangeStartUnselected(date);
                        }
                        validateAndUpdate();
                        return false;
                    }
                    ...
                    clearOldSelections();
                } else { // weipf do not care when second select date is before current one
...
//                    }
            ...
            case SINGLE:
                // weipf: unselect the current one if it is already in selected
        ...
        if (date != null) {
            ...
            if (selectionMode == SelectionMode.RANGE && selectedCells.size() == 1) {
                ...
                if (dateRangeListener != null) {
                    dateRangeListener.onRangeStartSelected(date);
                }
            }
            ...
            if (selectionMode == SelectionMode.RANGE && selectedCells.size() > 1) {
                ...
                if (end.before(start)) { // weipf 开始时间晚于结束时间时，对调位置
                ...
                if (dateRangeListener != null) {
                    dateRangeListener.onRangeEndSelected(start, end);
                }
    ...
    private class MonthAdapter extends RecyclerView.Adapter<MonthViewHolder> implements
            StickyRecyclerHeadersAdapter<MonthHeaderViewHolder> {
...
//            monthView.setTag(R.id.day_view_adapter_class, dayViewAdapter.getClass());
            monthView.setCellAspectRatio(aspectRatio);
        ...
        public void onBindViewHolder(MonthViewHolder holder, int position) {
            ...
            monthView.setCellViewAdapter(monthCellViewAdapter);
...
//        }
        ...
        public long getHeaderId(int position) {
            return months.get(position).toInteger();
        }

        @Override
        public MonthHeaderViewHolder onCreateHeaderViewHolder(ViewGroup parent) {
            View view = inflater.inflate(R.layout.view_calendar_header_month, parent, false);
            return new MonthHeaderViewHolder(view);
        }

        @Override
        public void onBindHeaderViewHolder(MonthHeaderViewHolder holder, int position) {
            holder.label.setText(months.get(position).getLabel());
        }
...
//        }
    ...
    private void callBindListener(MonthViewHolder holder, MonthDescriptor month, List<List<MonthCellDescriptor>> cells,
            int position) {
        if (this.onBindMonthViewListener != null) {
            this.onBindMonthViewListener.onBindMonthView(holder, month, cells, position);
        }
    }

    public MonthDescriptor getMonth(int position) {
        return months.get(position);
    }

    public List<List<MonthCellDescriptor>> getMonthCells(int position) {
        return cells.get(position);
    }

    public void setOnBindMonthViewListener(OnBindMonthViewListener listener) {
        this.onBindMonthViewListener = listener;
    }

    public void setMonthCellViewAdapter(MonthView.CellViewAdapter adapter) {
        this.monthCellViewAdapter = adapter;
    }

    public interface OnBindMonthViewListener {
        void onBindMonthView(MonthViewHolder holder, MonthDescriptor month, List<List<MonthCellDescriptor>> cells,
                int position);
    }
    ...
    private class MonthHeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        public MonthHeaderViewHolder(View itemView) {
            super(itemView);
            label = (TextView) itemView.findViewById(R.id.tv_calendar_header_month);
        }
    }
    ...
    List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
        ...
        while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
            ...
            for (int c = 0; c < 7; c++) {
                ...
                boolean isHoliday = dayOfWeek == Calendar.SUNDAY || dayOfWeek == Calendar.SATURDAY;
                ...
                if (selectedCals.size() > 1) {
                    if (sameDate(minSelectedCal, cal)) {
                        ...
                        if (sameDate(maxDate(selectedCals), cal)) {
                            rangeState = RangeState.LAST;
                        } else {
                            if (betweenDates(cal, minSelectedCal, maxSelectedCal)) {
                ...
                MonthCellDescriptor cellDescriptor = new MonthCellDescriptor(date, isCurrentMonth, isSelectable, isSelected, isToday, isHighlighted,
                        isHoliday, value, rangeState);
                cellDescriptor.setAfterToday(date.after(today.getTime()));
    ...
    public void setOnDateRangeSelectedListener(OnDateRangeSelectListener listener) {
    ...
    public void setTimeClock(ITimeClock clock) {
        if (clock == null) {
            return;
        }
        this.clock = clock;
        ...
        today.setTimeInMillis(this.clock.currentTimeMillis());
    }

    /**
     * set if show inactive days, if true, show them with different style.
     * if false, hide them.
     * @param showInactiveDay
     */
    public void setShowInactiveDay(boolean showInactiveDay) {
        this.showInactiveDays = showInactiveDay;
    }
    ...
    public interface OnDateRangeSelectListener {
        ...
        void onRangeEndSelected(Date start, Date end);

        void onRangeStartUnselected(Date start);
    }
    ...
    public interface CellClickInterceptor {
        boolean onCellClicked(MonthCellDescriptor cell, Date date);
    }
