...
- (void)layoutSubviews
{
    ...
    if (_subtitle) {
        ...
        _subtitleLabel.frame = CGRectMake(
                                          ...
                                          (_titleLabel.fs_bottom-self.preferredTitleOffset.y) - (_titleLabel.fs_height-_titleLabel.font.pointSize)+self.preferredSubtitleOffset.y + 2,
                                          self.contentView.fs_width,
                                          subtitleHeight
                                          );
    } else {
    ...
    diameter = ceilf([UIScreen mainScreen].bounds.size.width/7) - 4;
...
- (UIColor *)colorForCurrentStateInDictionary:(NSDictionary *)dictionary
{
    ...
    if (self.placeholder && [[dictionary allKeys] containsObject:@(FSCalendarCellStatePlaceholder)]) {
        ...
        //[UIColor yellowColor];
        //dictionary[@(FSCalendarCellStatePlaceholder)];
    }
...
- (UIColor *)colorForTitleLabel
{
    ...
    if (self.isFestival && !self.placeholder) {
        return [UIColor shy_colorWithHex:0xfd777c];
    }
    if (self.placeholder) {
        if (self.isFromScope) {
...
- (CGFloat)borderRadius
{
    //@hx设置 border的radius
    return 0.2;
    //_preferredBorderRadius >= 0 ? _preferredBorderRadius : _appearance.borderRadius;
}
