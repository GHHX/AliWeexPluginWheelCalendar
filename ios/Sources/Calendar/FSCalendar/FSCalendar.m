...
@property (assign, nonatomic) BOOL isAllowSelected;
...
{
    NSDate *date = MVP_GET_CORRECT_DATE;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.isFromScope = self.isFilterAllowSelectNoSubtitle;
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([cell isKindOfClass:[FSCalendarCell class]] && cell.subtitle.length <= 0 && _isFilterAllowSelectNoSubtitle) {
        _isAllowSelected = NO;
        if (_delegateProxy && [_delegateProxy respondsToSelector:@selector(calendarCancelByBiz)]) {
            [_delegateProxy calendarCancelByBiz];
        }
        return NO;
    }
    FSCalendarMonthPosition monthPosition = [self.calculator monthPositionForIndexPath:indexPath];
...
{
    if (!_isAllowSelected && _isFilterAllowSelectNoSubtitle) {
        return;
    }
    ...
    if (_weekdayHeight == FSCalendarAutomaticDimension) {
        if (_preferredWeekdayHeight == FSCalendarAutomaticDimension) {
            if (!self.floatingMode) {
                ...
                //@hx设置header中week的高度
                _preferredWeekdayHeight = 0;
                //FSCalendarStandardWeekdayHeight*MAX(1, FSCalendarDeviceIsIPad*1.5)*_lineHeightMultiplier;
            }
        }
...
- (void)reloadDataForCell:(FSCalendarCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *festival = [MVPCalendarTransformFestival transformFestivalWithDate:date];
    if (_isPreSale && [MVPCalendarTransformFestival isPreSaleDate:date preNum:_preNum]) {
        cell.isPreSale = YES;
    }else{
        cell.isPreSale = NO;
    }
